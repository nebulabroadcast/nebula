...
(function (root, factory) {
  ...
  function messageHasSubscribers(message) {
    var topic = String(message),
      found =
        hasDirectSubscribersFor(topic) || hasDirectSubscribersFor(ALL_SUBSCRIBING_MSG),
      position = topic.lastIndexOf('.');
  ...
  PubSub.subscribe = function (message, func) {
    ...
    if (!Object.prototype.hasOwnProperty.call(messages, message)) {
      ...
      if (onSubscriptionsChange) {
        onSubscriptionsChange(Object.keys(messages));
      }
    }
  ...
  PubSub.unsubscribe = function (value) {
    ...
    for (const key in messages) {
      if (!(messages[key] && Object.keys(messages[key]).length)) {
        delete messages[key];
      }
    }
    if (onSubscriptionsChange) {
  ...
  PubSub.setOnSubscriptionsChange = function (fn) {
